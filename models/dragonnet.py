...
class Dragonnet(nn.Module):
    def __init__(self, backbone: nn.Module, feature_dim: int, dropout_p: float = 0.2):
        ...
        self.predictor_y1 = MLP(feature_dim, 1, 2, shrink=2, dropout_p=dropout_p)
        self.predictor_y0 = MLP(feature_dim, 1, 2, shrink=2, dropout_p=dropout_p)

        self.dropout = nn.Dropout(dropout_p) if dropout_p > 0 else None
    
    def forward(self, inputs):
        z = self.backbone(inputs)
        t = self.predictor_t(z).squeeze(1)
        y1 = torch.sigmoid(self.predictor_y1(z)).squeeze(1)
        y0 = torch.sigmoid(self.predictor_y0(z)).squeeze(1)
